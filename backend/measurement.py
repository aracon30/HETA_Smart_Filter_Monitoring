...
class MeasurementLoop:
    ...
    def run(self):
        ...
        while self._state["running"]:
            ...
            if profile_valid and profile:
                r_eff_clean_ref = profile.get("reference_r_eff_start") or profile.get("reference_r_eff")
            ...
            if r_eff_clean_ref and r_eff_limit_ref and fs.r_eff > 0:
                raw_health, _ = calculate_filter_health_from_r_eff(fs.r_eff, r_eff_clean_ref, r_eff_limit_ref)
            else:
            ...
            flow_ok = fs.flow_l_min >= flow_thr and fs.dp_bar >= dp_clean * 0.5 and not sensor_error
            now_ts = time.time()
            ...
            if sim_mode:
                ...
                if flow_ok:
                    ...
                    if now_ts - self.mstate.flow_stable_since >= stab_secs:
                        ...
                        self._db.insert_service_event(
                            "ZYKLUS_PAUSE_ENDE", heta_code, f"{op_label} nach {pause_dur:.0f} s beendet"
                        )
                        with self._state_lock:
                            self._state["cycle_paused"] = False
                            self._state["cycle_pause_start_time"] = None
                    ...
                    if now_ts - pause_start >= max_pause:
                        ...
                        self._learning.abort_cycle()
                        self._db.insert_service_event(
                            "ZYKLUS_ABGEBROCHEN", heta_code, f"Kein Durchfluss seit {max_pause / 86400:.0f} Tagen"
                        )
                        with self._state_lock:
                            self._state["cycle_paused"] = False
                            self._state["cycle_active"] = False
                ...
                if cycle_paused:
                    if self._display:
                        ...
                        self._display.show_cycle_paused(self._state.get("cycle_active_seconds", 0.0), pause_secs)
                    with self._state_lock:
                        self._state["last_update"] = time.strftime("%Y-%m-%dT%H:%M:%S")
                    time.sleep(interval)
                    continue
                ...
                if flow_ok:
                    self.mstate.flow_below_since = None
                    with self._state_lock:
                        self._state["cycle_active_seconds"] = self._state.get("cycle_active_seconds", 0.0) + interval
                else:
                    if self.mstate.flow_below_since is None:
                        ...
                        # Zyklus pausieren
                        op_label = "Batch-Pause" if self._settings.get("operation_mode") == "batch" else "Unterbrechung"
                        self._learning.add_event(
                            "PAUSE_START", f"{op_label} gestartet – Q={fs.flow_l_min:.1f} l/min", category="pause"
                        )
                        self._db.insert_service_event(
                            "ZYKLUS_PAUSE_START",
                            heta_code,
                        ...
                        cycle_paused = True
                        logger.info("Zyklus pausiert – Q=%.1f l/min < Schwellwert %.1f l/min.", fs.flow_l_min, flow_thr)
            ...
            if cycle_active and cycle_start_ts and (time.time() - cycle_start_ts) < 10:
                ...
                if anomaly and not was_anomaly:
                    self._learning.add_event(
                        "WARNUNG", f"Startverhalten-Anomalie: R_eff {anom_pct:+.1f}% zur Referenz", category="anomaly"
                    )
            ...
            if fs.status != prev_status and cycle_active:
                if fs.status == STATUS_FEHLER:
                    ...
                    self._learning.add_event(
                        "WECHSEL", f"Filterwechsel erforderlich – Δp={fs.dp_bar:.3f} bar", category="status"
                    )
            ...
            if fs.dp_bar >= dp_limit and not awaiting and cycle_active:
                logger.warning("Filterwechsel-Grenzwert überschritten! dp=%.3f >= %.2f", fs.dp_bar, dp_limit)
                with self._state_lock:
                ...
                self._state.update(
                    {
                        ...
                        "temperature_c": fs.temperature_c,
                        "r_eff": fs.r_eff,
                        "r_rel_factor": round(fs.r_eff / r_eff_clean_ref, 4)
                        if (r_eff_clean_ref and r_eff_clean_ref > 0)
                        else None,
                        "filter_health_percent": smoothed_health,
                        ...
                        "reference_dp_clean": (_loop_profile.get("reference_dp_clean") or 0.0)
                        if _loop_profile
                        else 0.0,
    ...
    def confirm_filter_change(self):
        ...
        self._db.insert_service_event("FILTERWECHSEL_BESTAETIGT", heta_code, json.dumps({"timestamp": time.time()}))
