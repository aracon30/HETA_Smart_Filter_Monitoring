...
def run_diagnostics(
    ...
    checks.append(
        {
            ...
            "status": "ok",
            "detail": (
                f"Python {sys.version.split()[0]}  ·  {platform.machine()}  ·  {platform.system()} {platform.release()}"
            ),
            "hints": [],
        }
    )
    ...
    if sim_mode:
        ...
        mode_hints = ["Für Echtbetrieb: simulation_mode=false in den Einstellungen"] if hw_avail else []
    else:
        if sensor_fault:
            mode_status = "error"
        ...
        checks.append(
            {
                ...
                "status": "ok",
                "detail": (f"{meas_count} Messwerte  ·  {cycle_count} Filterzyklen  ·  Pfad: {db.db_path}"),
                "hints": [],
            }
        )
        ...
        checks.append(
            {
                ...
                "status": "ok",
                "detail": (
                    f"Verbunden mit {settings.get('mqtt_broker', 'localhost')}:"
        ...
        checks.append(
            {
                ...
                "status": "warning",
                "detail": (
                    f"Nicht verbunden mit {settings.get('mqtt_broker', 'localhost')}:{settings.get('mqtt_port', 1883)}"
                ),
                "hints": [
        ...
        checks.append(
            {
                ...
                "status": "ok",
                "detail": (
                    f"Server aktiv auf {settings.get('modbus_host', '0.0.0.0')}:"
        ...
        checks.append(
            {
                ...
                "status": "warning",
                "detail": (
                    f"Server nicht aktiv  ({settings.get('modbus_host', '0.0.0.0')}:{settings.get('modbus_port', 502)})"
                ),
                "hints": [
        ...
        checks.append(
            {
                ...
                "status": "info",
                "detail": (
                    "Simulationsmodus – Sensor-Hardware nicht geprüft"
                    + ("  ·  AnoPi Shield erreichbar" if hw_avail else "  ·  kein AnoPi Shield erkannt")
                ),
                "hints": (["Für Echtbetrieb: simulation_mode=false in den Einstellungen"] if hw_avail else []),
            }
        )
    elif not hw_avail:
        checks.append(
